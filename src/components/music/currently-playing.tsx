...
export function CurrentlyPlaying() {
  ...
  useEffect(() => {
    const fetchMostPlayedMusic = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/music/most-played?count=4`)
        const data = await response.json()
        setMusic(data?.music || [])
      } catch (error) {
        console.error("Error fetching most played music:", error)
      }
    }
        ...
        {music.map((item) => (
          ...
          >
            <div className="relative aspect-square w-full h-full rounded-md overflow-hidden">
              ...
              />
              <div className="absolute top-1 right-1 bg-black/70 text-white px-2 py-1 rounded-md text-xs">
